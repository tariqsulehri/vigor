...
namespace VIGOR.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private FinFescalYearRepository finFescalYearRepository;
        private User user;
        ...
        public AccountController()
        {
            ...
            finFescalYearRepository = new FinFescalYearRepository();
            user = new User();

            user.Id = 1;
            user.UserName = "Shehzad Yaseen";
            comp.Name = "Vigour Impex";
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            // var result = _userRepository.GetAllUsers().Any(x => x.UserName.Trim() == model.UserName.Trim() && x.Password.Trim() == model.Password.Trim());
            var result = await SignInManager.PasswordSignInAsync(model.UserName, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                    Session["UserModel"] = user;
                    Session["CompanyModel"] = comp;
                    int userId = _userRepository.GetAllUsers().First(x => x.UserName.Trim() == model.UserName.Trim() && x.Password.Trim() == model.Password.Trim()).Id;
                    if (userId != 0)
                    {
                        FormsAuthentication.SetAuthCookie(userId.ToString(),false);
                        LoggedinUser.LoggedInUser = _userRepository.FindById(userId);
                        LoggedinUser.Company = companyRepository.FindById(2);
                        LoggedinUser.CurrentFiscalYear = finFescalYearRepository.FindById(1);

                        Session["UserId"] = userId;
                        Session["UserPrevilages"] = db.UserPrevilages.Where(x => x.UserId == userId).Select(x => x.OptionId).ToList();
        ...
        {
            //ViewBag.EMP_ID = new SelectList(db.PAY_SM_EMPLOYEE.Where(x => x.Id == null), "EMP_ID", "EMP_NIC_NO");
            return View();
        }
