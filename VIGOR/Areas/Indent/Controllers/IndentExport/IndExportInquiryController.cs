    ...
    public class IndExportInquiryController : Controller
    {
            ...
            Int32? DepartmentID, Int32? CustomerId, string Customer)
        {
            ViewBag.IndExportInquiryList = _indExportInquiryRepository.GetAllExportInquires()
                .Where(a => a.CommodityTypeId.Equals(commodityId) || commodityId.Equals(0) || commodityId.Equals(null) && a.IsClosed != "Y")
                .Where(a => a.DepartmentID.Equals(DepartmentID) || DepartmentID.Equals(0) || DepartmentID.Equals(null) && a.IsClosed != "Y")
                .Where(a => a.CustomerId.Equals(CustomerId) || CustomerId.Equals(0) || CustomerId.Equals(null) && a.IsClosed != "Y")
                .Where(a => a.Customer.Trim().Equals(Customer) || Customer.Equals(string.Empty) || Customer.Equals(null) && a.IsClosed != "Y")
                .Where(a => a.InquiryDate >= fromDate && a.InquiryDate <= toDate).ToList();

            TempData["DepartmentID"] = DepartmentID;
            TempData["CommodityID"] = commodityId;
        ...
        public ActionResult Create(IndExportInquiry model)
        {
            ...
            return View(model);
            //return RedirectToAction("Create", new { DepartmentID = model.DepartmentID, CommodityTypeId = model.CommodityTypeId});
        }
                ...
                foreach (var Key in indExportInquiryDetailList)
                {
                    ...
                    if (Request.Form.AllKeys.Any(k => k == "det[" + index + "][ProductId]"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(Request.Form["det[" + index + "][ProductId]"]))
                        {
                            ...
                            _IndExportInquiryDetail.CostingSheetRef = Convert.ToString(Request.Form["det[" + index + "][CostingSheetRef]"]);
                            if (!string.IsNullOrEmpty(Request.Form["det[" + index + "][CostingPrice]"]))
                            {
                                _IndExportInquiryDetail.CostingPrice = Convert.ToDecimal(Request.Form["det[" + index + "][CostingPrice]"]);
                            }
                            _IndExportInquiryDetail.InquiryLineItemRemarks = Convert.ToString(Request.Form["det[" + index + "][Remarks]"]);
                        ...
                        if (!string.IsNullOrEmpty(Request.Form["det[" + index + "][ProductId]"]))
                        {
                            ...
                            if (index == "0")
                            {
                                if (!string.IsNullOrEmpty(Request.Form["det[" + index + "][OfferCustomerId1]"]))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(Request.Form["det[" + index + "][Remarks]"]))
                                    {
                                        _indExportInquiryOffer.Remarks = Convert.ToString(Request.Form["det[" + index + "][Remarks]"]);
                                    }
                                    else
                                    {
                                        _indExportInquiryOffer.Remarks = " .";
                                    }
        ...
        private IndExportInquiryOffer AssignValueIndExportInquiryOffer(IndExportInquiryOffer model)
        {
            ...
            _indExportInquiryOffer.OfferedBy = model.CustomerId.ToString();
