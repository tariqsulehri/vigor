            ...
            if (config['countRow']) {
                if (config['countRowAdd']) {
                    ...
                    $(table).find('tr:not(.noedit)').each(function (index, elem) {
                        var ind = index + 1;
                        $(elem).prepend('<td class="tdpading">' + ind + '</td>');
                    });
                }
            }
        ...
        function addRowWithTab() {
            $(table).on("keydown", "tr:not(.noedit):last a.delete", function (e) {
                if (e.keyCode == 9) {
                    ...
                    //$(table).find('.bind-select2-grid').select2("destroy");
                    addRow();
		...
		*/
        ...
        function addRow() {
            ...
            if ($(tr).find("input, select, textarea").length > 0) {
                ...
                if ($(tr).find("select").get(0)) {
                    $(tr).find("select").get(0).selectedIndex = 0;
                }
                if ($(tr).find("select").get(1)) {
                    $(tr).find("select").get(1).selectedIndex = 0;
                }
                if ($(tr).find("input:text").prop("disabled", true)) {
            ...
            //$(table).find('.bind-select2-grid').select2({});
        }
        ...
        function delRow(elem) {
            ...
            if ($(table).find('tr:not(.noedit)').length > 1) {
                ...
                if ($(table).find('tr:not(.noedit)').length == 1) {
                    var tr = $(table).find('tr:not(.noedit):first');
